...
st.set_page_config(page_title="🌱 Thriving Communities", page_icon="🌱", layout="wide")
...
.chip-tag { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 1px; background: rgba(56,239,125,0.2); color: #38ef7d; border: 1px solid rgba(56,239,125,0.4); margin-bottom: 16px; }
.headline-insight { background: linear-gradient(135deg, rgba(56,239,125,0.08), rgba(56,239,125,0.03)); border: 1px solid rgba(56,239,125,0.25); border-left: 4px solid #38ef7d; border-radius: 12px; padding: 18px 24px; margin-bottom: 28px; font-size: 15px; color: rgba(255,255,255,0.9); line-height: 1.6; }
...
.year-badge { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 11px; font-weight: 700; background: rgba(56,239,125,0.2); color: #38ef7d; border: 1px solid rgba(56,239,125,0.3); margin-left: 8px; vertical-align: middle; }
</style>
...
selected_year, compare_year, show_ohio, selected_charts = render_sidebar(
    page_name="🌱 Thriving Communities",
    all_data=all_data,
    chart_options=["Life Expectancy Trend", "YPLL Trend", "Health Outcomes Comparison", "County Ranking"]
)
...
st.markdown(f"# 🌱 Thriving Communities — Health Outcomes <span class='year-badge'>Showing: {selected_year}</span>", unsafe_allow_html=True)
st.markdown('<div class="chip-tag">🌱 CHIP PRIORITY 4</div>', unsafe_allow_html=True)
st.markdown("Life expectancy, mortality, and overall health outcomes for Wood County vs Ohio.")

st.markdown(f"""
<div class="headline-insight">
    💡 <strong>Key Finding ({selected_year}):</strong> Wood County residents live an average of
    <strong>{life_exp_h} years</strong> — {life_diff} years {'longer' if life_diff > 0 else 'shorter'} than the Ohio average of {life_exp_o}.
    The county also loses <strong>{ypll_diff:,} {'fewer' if ypll_diff > 0 else 'more'} years of potential life</strong> per 100k residents
    than the state average — a reflection of overall community health and resilience aligned with CHIP Priority 4.
</div>
""", unsafe_allow_html=True)

render_ai_banner("Thriving Communities & Health Outcomes")
...
def make_trend_chart(trend_df, col, color_map, y_label, value_suffix=""):
    ...
    if df.empty:
        return None
    fig = px.scatter(df, x='year', y=col, color='geography', color_discrete_map=color_map, template='plotly_dark')
...
if "Life Expectancy Trend" in selected_charts:
    ...
    col = find_column(all_data['additional'], ['Life Expectancy'])
    if col:
        fig = make_trend_chart(get_trend(all_data['additional'], col), col,
                               {'Wood County': '#38ef7d', 'Ohio': '#4ECDC4'}, 'Life Expectancy', ' yrs')
        if fig:
...
if "YPLL Trend" in selected_charts:
    ...
    col = find_column(all_data['select'], ['Years of Potential Life Lost Rate', 'YPLL Rate'])
    if col:
        ...
        if not df.empty:
            fig2 = px.area(df, x='year', y=col, color='geography',
                           color_discrete_map={'Wood County': '#38ef7d', 'Ohio': '#4ECDC4'}, template='plotly_dark')
...
if "Health Outcomes Comparison" in selected_charts:
    st.markdown(f'<div class="section-title">Health Outcomes — Wood County vs Ohio ({selected_year})</div>', unsafe_allow_html=True)
    compare_df = pd.DataFrame({
        'Indicator': ['Injury Death Rate', 'Infant Mortality Rate', 'Child Mortality Rate', 'Low Birth Weight %'],
        'Wood County': [injury_h, infant_h, child_mort_h, lbw_h],
    ...
    fig3 = px.bar(melted, x='Indicator', y='Value', color='Geography', barmode='group',
                  color_discrete_map={'Wood County': '#38ef7d', 'Ohio': '#4ECDC4'},
...
if "County Ranking" in selected_charts:
    st.markdown('<div class="section-title">Life Expectancy — Wood County Ranked Among All 88 Ohio Counties</div>', unsafe_allow_html=True)
    ...
    if not all_counties.empty and 'Life Expectancy' in all_counties.columns:
        ...
        wood_idx = counties_clean[counties_clean['County'] == 'Wood'].index
        wood_rank = total - wood_idx[0] if len(wood_idx) > 0 else None
        counties_clean['highlight'] = counties_clean['County'].apply(
            lambda x: 'Wood County' if x == 'Wood' else 'Other Ohio Counties'
        )
        fig4 = px.bar(counties_clean, x='Life Expectancy', y='County', orientation='h',
                      color='highlight',
                      color_discrete_map={'Wood County': '#38ef7d', 'Other Ohio Counties': '#1D6B8A'},
        ...
        fig4.update_layout(**LAYOUT_BASE, height=2400, yaxis=dict(tickfont=dict(size=10)),
                           xaxis=dict(range=[66, 82]), bargap=0.12)
        if wood_rank:
            st.markdown(f"Wood County ranks **#{wood_rank} out of {total}** Ohio counties — highlighted in green.")
        st.plotly_chart(fig4, use_container_width=True, config=CHART_CONFIG)

st.markdown("---")
export_df = pd.DataFrame({
    'Indicator': ['Life Expectancy (years)', 'YPLL Rate (per 100k)', 'Injury Death Rate', 'Infant Mortality Rate', 'Child Mortality Rate', 'Low Birth Weight %'],
    'Wood County': [life_exp_h, ypll_h, injury_h, infant_h, child_mort_h, lbw_h],
...
st.download_button(label="📥 Download Health Outcomes Data as CSV", data=csv,
                   file_name=f"wood_health_outcomes_{selected_year}.csv", mime="text/csv")
if st.checkbox("Show raw comparison data"):
    st.dataframe(export_df, use_container_width=True)

render_disclaimer("Thriving Communities & Health Outcomes")
render_sidebar_chat("Thriving Communities & Health Outcomes")
