...
.headline-insight { background: linear-gradient(135deg, rgba(168,85,247,0.08), rgba(168,85,247,0.03)); border: 1px solid rgba(168,85,247,0.3); border-left: 4px solid #a855f7; border-radius: 12px; padding: 18px 24px; margin-bottom: 28px; font-size: 15px; color: rgba(255,255,255,0.9); line-height: 1.6; }
...
.year-badge { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 11px; font-weight: 700; background: rgba(168,85,247,0.2); color: #a855f7; border: 1px solid rgba(168,85,247,0.3); margin-left: 8px; vertical-align: middle; }
</style>
...
selected_year, compare_year, show_ohio, selected_charts = render_sidebar(
    ...
    all_data=all_data,
    chart_options=["Population Composition", "Age Distribution", "Race & Ethnicity", "Income by Race", "Single-Parent Households Trend", "Disability & Language"]
)
...
st.markdown(f"# 👥 Demographics & Population Profile <span class='year-badge'>Showing: {selected_year}</span>", unsafe_allow_html=True)
st.markdown('<div class="chip-tag">📊 POPULATION CONTEXT — ALL CHIP PRIORITIES</div>', unsafe_allow_html=True)
st.markdown("Who lives in Wood County — age, race, income, and community characteristics.")

st.markdown(f"""
<div class="headline-insight">
    💡 <strong>Population Profile ({selected_year}):</strong> Wood County has a population of
    <strong>{int(pop_h):,}</strong> residents. Home to Bowling Green State University,
    the county has a younger, more educated demographic than many Ohio counties.
    Rural population stands at <strong>{safe(pct_rural_h, '%')}</strong> vs {safe(pct_rural_o, '%')} for Ohio.
    Understanding Wood County's demographics is essential context for all four CHIP priorities.
</div>
""", unsafe_allow_html=True)
...
<div class="kpi-container">
    <div class="kpi-card purple">
        ...
        <div class="kpi-sub">Ohio: {int(pop_o):,} · Wood is {round(pop_h/pop_o*100, 2)}% of state</div>
...
if "Population Composition" in selected_charts:
    st.markdown(f'<div class="section-title">Population Composition — Wood County vs Ohio ({selected_year})</div>', unsafe_allow_html=True)
    comp_data = pd.DataFrame({
        'Category': ['Under 18', '65 and Older', 'Rural', 'Female', 'Disabled', 'Single-Parent HH'],
        'Wood County': [pct_under18_h, pct_65plus_h, pct_rural_h, pct_female_h, pct_disabled_h, pct_singlepar_h],
...
if "Age Distribution" in selected_charts:
    st.markdown('<div class="section-title">Age Distribution — Wood County</div>', unsafe_allow_html=True)
    pct_working = round(100 - (pct_under18_h or 0) - (pct_65plus_h or 0), 1)
    age_df = pd.DataFrame({'Age Group': ['Under 18', 'Working Age (18–64)', '65 and Older'],
                           'Percentage': [pct_under18_h or 0, pct_working, pct_65plus_h or 0]})
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Wood County**")
        fig2a = px.pie(age_df, values='Percentage', names='Age Group',
                       color_discrete_sequence=['#a855f7', '#4ECDC4', '#ffd200'], template='plotly_dark', hole=0.45)
        fig2a.update_traces(hovertemplate='<b>%{label}</b><br>%{value:.1f}%<extra></extra>', textinfo='label+percent')
        fig2a.update_layout(paper_bgcolor='rgba(0,0,0,0)', showlegend=True, legend=dict(orientation='h', y=-0.2), margin=dict(t=20, b=20))
    ...
    if show_ohio:
        with col2:
            ...
            age_df_o = pd.DataFrame({'Age Group': ['Under 18', 'Working Age (18–64)', '65 and Older'],
            ...
            fig2b.update_layout(paper_bgcolor='rgba(0,0,0,0)', showlegend=True, legend=dict(orientation='h', y=-0.2), margin=dict(t=20, b=20))
            st.plotly_chart(fig2b, use_container_width=True, config=CHART_CONFIG)

if "Race & Ethnicity" in selected_charts:
    st.markdown(f'<div class="section-title">Race & Ethnicity — Wood County vs Ohio ({selected_year})</div>', unsafe_allow_html=True)
    race_data = pd.DataFrame({
        'Group': ['Non-Hispanic White', 'Non-Hispanic Black', 'Hispanic', 'Other/Mixed'],
        'Wood County': [pct_white_h or 0, pct_black_h or 0, pct_hispanic_h or 0,
...
if "Income by Race" in selected_charts:
    ...
    income_race_cols = {'White': 'Household Income (White)', 'Black': 'Household Income (Black)',
                        'Hispanic': 'Household Income (Hispanic)', 'Asian': 'Household Income (Asian)', 'AIAN': 'Household Income (AIAN)'}
    income_race_data = []
    ...
    if income_race_data:
        ...
        fig4 = px.bar(income_race_df, x='Median Income ($)', y='Race/Ethnicity', orientation='h',
                      ...
                      labels={'Median Income ($)': 'Median Household Income ($)', 'Race/Ethnicity': ''}, template='plotly_dark')
        fig4.update_traces(hovertemplate='<b>%{y}</b><br>Median Income: $%{x:,.0f}<extra></extra>')
        fig4.update_layout(**LAYOUT_BASE, coloraxis_showscale=False, xaxis=dict(tickprefix='$', tickformat=','))
        if income_h:
            fig4.add_vline(x=income_h, line_dash='dash', line_color='#a855f7', opacity=0.8,
                           annotation_text=f'Wood County: ${int(income_h):,}',
    ...
    if col:
        ...
        if not df.empty:
            fig5 = px.scatter(df, x='year', y=col, color='geography',
                              color_discrete_map={'Wood County': '#a855f7', 'Ohio': '#4ECDC4'}, template='plotly_dark')
            ...
            fig5.update_layout(**LAYOUT_BASE, xaxis=dict(tickformat='d', dtick=1), yaxis=dict(title='Children in Single-Parent HH (%)'))
...
if "Disability & Language" in selected_charts:
    st.markdown(f'<div class="section-title">Disability & Language Access — Wood County vs Ohio ({selected_year})</div>', unsafe_allow_html=True)
    access_data = pd.DataFrame({
        'Indicator': ['Disability Rate %', 'Not Proficient in English %'],
        'Wood County': [pct_disabled_h or 0, pct_noenglish_h or 0],
...
export_df = pd.DataFrame({
    'Indicator': ['Total Population', 'Rural %', 'Female %', 'Under 18 %', '65 and Older %',
                  ...
                  'Disability Rate %', 'Not Proficient in English %', 'Single-Parent Households %', 'Median Household Income'],
    'Wood County': [int(pop_h) if pop_h else None, pct_rural_h, pct_female_h, pct_under18_h,
...
st.download_button(label="📥 Download Demographics Data as CSV", data=csv,
                   file_name=f"wood_demographics_{selected_year}.csv", mime="text/csv")
