...
.chip-tag { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 1px; background: rgba(168,85,247,0.2); color: #a855f7; border: 1px solid rgba(168,85,247,0.4); margin-bottom: 16px; }
.headline-insight { background: linear-gradient(135deg, rgba(168,85,247,0.1), rgba(168,85,247,0.05)); border: 1px solid rgba(168,85,247,0.3); border-left: 4px solid #a855f7; border-radius: 12px; padding: 18px 24px; margin-bottom: 28px; font-size: 15px; color: rgba(255,255,255,0.9); line-height: 1.6; }
...
scenario = st.sidebar.radio("Select Scenario", options=["🟢 Optimistic", "🟡 Moderate", "🔴 Pessimistic"], index=1)
target_year = st.sidebar.slider("Project to Year", min_value=2026, max_value=2040, value=2030, step=1)
selected_metrics = st.sidebar.multiselect(
    "Metrics to Project",
    options=["Drug Overdose Rate", "Life Expectancy", "Adult Obesity", "Mental Health Providers", "Children in Poverty", "Physical Inactivity"],
    default=["Drug Overdose Rate", "Life Expectancy", "Adult Obesity", "Mental Health Providers", "Children in Poverty", "Physical Inactivity"]
)
...
scenarios = {
    "🟢 Optimistic": {"color": "#38ef7d", "label": "Aggressive CHIP implementation — strong investment in all 4 priority areas",
                      "rates": {"Drug Overdose Rate": -0.05, "Life Expectancy": +0.008, "Adult Obesity": -0.02, "Mental Health Providers": +0.04, "Children in Poverty": -0.03, "Physical Inactivity": -0.025}},
    "🟡 Moderate": {"color": "#ffd200", "label": "Current pace — partial CHIP implementation, steady progress",
                    "rates": {"Drug Overdose Rate": -0.02, "Life Expectancy": +0.003, "Adult Obesity": +0.005, "Mental Health Providers": +0.015, "Children in Poverty": -0.01, "Physical Inactivity": +0.005}},
    "🔴 Pessimistic": {"color": "#ff416c", "label": "Resource constraints — CHIP underfunded, trends worsen",
                       "rates": {"Drug Overdose Rate": +0.03, "Life Expectancy": -0.002, "Adult Obesity": +0.02, "Mental Health Providers": -0.01, "Children in Poverty": +0.02, "Physical Inactivity": +0.02}}
}
...
units = {"Drug Overdose Rate": "per 100k", "Life Expectancy": "years", "Adult Obesity": "%", "Mental Health Providers": "per 100k", "Children in Poverty": "%", "Physical Inactivity": "%"}
lower_is_better = {"Drug Overdose Rate": True, "Life Expectancy": False, "Adult Obesity": True, "Mental Health Providers": False, "Children in Poverty": True, "Physical Inactivity": True}
card_colors = {"Drug Overdose Rate": "red", "Life Expectancy": "teal", "Adult Obesity": "amber", "Mental Health Providers": "purple", "Children in Poverty": "green", "Physical Inactivity": "amber"}
...
st.markdown(f"Projecting Wood County health metrics to **{target_year}** under the **{scenario}** scenario.")
...
with col1:
    st.markdown('<div class="scenario-box scenario-optimistic"><strong>🟢 Optimistic</strong><br>Full CHIP implementation. Strong funding for all 4 priority areas including mental health, healthy living, maternal health, and thriving communities.</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="scenario-box scenario-moderate"><strong>🟡 Moderate</strong><br>Partial CHIP implementation. Current trends continue with incremental improvements across Wood County priorities.</div>', unsafe_allow_html=True)
with col3:
...
for row in rows:
    ...
    for m in row:
        base = base_values[m]; proj = projected[m]; unit = units[m]
        color = card_colors[m]; lib = lower_is_better[m]
        change = round(proj - base, 1); direction = '▼' if change < 0 else '▲'
        good = (change < 0 and lib) or (change > 0 and not lib); sentiment = '✅' if good else '⚠️'
        card_html += f"""<div class="kpi-card {color}"><div class="kpi-icon">{sentiment}</div>
            <div class="kpi-label">{m}</div>
            <div class="kpi-value">{proj} <span style="font-size:16px">{unit}</span></div>
            <div class="kpi-sub">2025 baseline: {base} {unit} · {direction} {abs(change)} projected change</div></div>"""
    card_html += '</div>'
...
for metric in selected_metrics:
    if metric not in base_values: continue
    st.markdown(f'<div class="section-title">{metric} — Projection to {target_year}</div>', unsafe_allow_html=True)
    base = base_values[metric]; unit = units[metric]; color = config['color']
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=projection_years + projection_years[::-1], y=opt_vals + pes_vals[::-1],
    ...
    fig.add_vline(x=2025, line_dash='dot', line_color='rgba(255,255,255,0.3)',
                  annotation_text='2025 Baseline', annotation_position='top right', annotation_font_color='rgba(255,255,255,0.5)')
    fig.update_layout(**LAYOUT_BASE, xaxis=dict(tickformat='d', dtick=1, title='Year'), yaxis=dict(title=f'{metric} ({unit})'))
...
for m in base_values:
    summary_rows.append({'Metric': m, 'Unit': units[m], '2025 Baseline': base_values[m],
...
st.download_button(label="📥 Download Forecast Data as CSV", data=csv,
                   file_name=f"wood_forecast_{scenario.replace(' ', '_')}_{target_year}.csv", mime="text/csv")

st.markdown("---")
st.markdown("""
<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-left:4px solid rgba(168,85,247,0.5);border-radius:10px;padding:16px 20px;margin-top:10px;">
    <p style="font-size:12px;color:rgba(255,255,255,0.5);margin:0 0 6px 0;font-weight:700;letter-spacing:1px;text-transform:uppercase;">📋 Forecast Disclaimer</p>
    <p style="font-size:12px;color:rgba(255,255,255,0.55);margin:0;line-height:1.7;">
        Projections are <strong>mathematical estimates only</strong> based on applying compound annual rates to 2025 baseline values from the
        <a href="https://www.countyhealthrankings.org/health-data/ohio" target="_blank" style="color:#a855f7;">County Health Rankings & Roadmaps</a> program.
        They are <strong>not official forecasts</strong>. Scenario rates are illustrative and aligned with the goals of the
        <a href="https://woodcountyhealth.org/home/reports-publications/" target="_blank" style="color:#a855f7;">2026–2028 Wood County CHIP</a>.
